...
export function DetailUser({ user, isOpen, setIsOpen }: DetailUserProps) {
  ...
  const renderRoles = (roles: string[]) => {
    if (!roles || roles.length === 0) return <span className="text-muted-foreground text-sm">Sin roles</span>;
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto bg-background text-foreground p-6">
        ...
        {isDetailLoading ? (
          ...
          <div className="grid gap-6 py-2">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center sm:items-start">
              <div className="flex-shrink-0 flex flex-col items-center">
                <Avatar className="w-24 h-24 border-2 border-muted">
                  <AvatarImage src={imageUrl} alt={detailedUser?.name} className="object-cover" />
                ...
                <div className="mt-2 text-center">
                  <Badge variant={detailedUser?.isActive ? "default" : "destructive"}>
            ...
            <div>
              <h3 className="text-sm font-medium mb-3 text-muted-foreground">Información Personal & Física</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="grid gap-1.5">
                  ...
                  <div className="p-2 border rounded-md bg-muted/10 text-sm truncate" title={formatDate(detailedUser?.birthDate)}>
             ...
             <div className="grid grid-cols-2 gap-4 text-xs text-muted-foreground mt-2 bg-muted/20 p-2 rounded">
                <p><strong>ID:</strong> <span className="font-mono">{detailedUser?._id}</span></p>
                <p><strong>Creado:</strong> {formatDate(detailedUser?.createdAt)}</p>
             </div>
