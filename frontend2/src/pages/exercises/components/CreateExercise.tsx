...
export function AddExercise({ isOpen, setIsOpen }: CreateExerciseProps) {
  ...
  const allMusculoOptions = MUSCULOS.map((m) => ({ value: m, label: m }));
  const materialOptions = MATERIALES.map((m) => ({ value: m, label: m }));
  const etiquetaOptions = ETIQUETAS.map((e) => ({ value: e, label: e }));
  const [videoFile, setVideoFile] = useState<File | null>(null);

  const watchedPrincipales = watch("musculosPrincipales") || [];
  const watchedSecundarios = watch("musculosSecundarios") || [];

  const filteredPrimaryOptions = allMusculoOptions.filter(
    (m) => !watchedSecundarios.includes(m.value),
  );

  const filteredSecondaryOptions = allMusculoOptions.filter(
    (m) => !watchedPrincipales.includes(m.value),
  );
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-3">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <ComboBoxMultiSelect
                    key={`primary-${watchedSecundarios.length}`}
